...
void ordenarArr(t_dato ad[TD]){
	...
	for (i = 0; ad[i].codigo != 0; i++)
	{
		for (j = i + 1; ad[j].codigo != 0; j++)
		{
			...
			{
				strcpy(auxStr, ad[i].nomArt);
				strcpy(ad[i].nomArt, ad[j].nomArt);
				strcpy(ad[j].nomArt, auxStr);
				
				aux = ad[i].cantidad;
				ad[i].cantidad = ad[j].cantidad;
				ad[j].cantidad = aux;
				
				auxF = ad[i].precio;
				ad[i].precio = ad[j].precio;
				ad[j].precio = auxF;
				
				aux = ad[i].codigo;
				ad[i].codigo = ad[j].codigo;
				ad[j].codigo = aux;
			}
		}
	}
